...
beforeAll(async () => {
  ...
  idTest = account.id;
  jwt = auth.signJWT(idTest, payload.name);
...
describe("testando as rotas de accounts/login", () => {
  ...
  it("POST /account - Deve retornar 201", async () => {
    ...
    const result = await request(app).post("/account").send(createPayload);
  ...
  it("POST /account - Deve retornar 400", async () => {
    const result = await request(app).post("/account");
  ...
  it("POST /account/login - Deve retornar 200", async () => {
    ...
    const result = await request(app).post("/account/login").send(loginPayload);
  ...
  it("POST /account/login - Deve retornar 400", async () => {
    const result = await request(app).post("/account/login");
